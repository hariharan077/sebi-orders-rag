#!/usr/bin/env python3
"""CLI entrypoint for refreshing structured SEBI directory/reference data."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Refresh stored structured SEBI directory/reference data.",
    )
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--run-migration",
        action="store_true",
        help="Apply the structured directory-reference SQL migrations before refresh.",
    )
    ...
    mode_group.add_argument(
        ...
        dest="apply",
        action="store_false",
        help="Fetch and parse without persisting database changes.",
    )
    mode_group.add_argument(
        "--apply",
        dest="apply",
        action="store_true",
        help="Persist the refreshed snapshots and rows. This is the default.",
    )
    ...
    parser.add_argument(
        ...
        default="all",
        help="Limit refresh to one official source family.",
    )
    return parser
...
def main() -> int:
    ...
    LOGGER.info(
        "Refreshing structured SEBI reference data in %s mode for source=%s",
    ...
    print("Structured directory refresh summary")
